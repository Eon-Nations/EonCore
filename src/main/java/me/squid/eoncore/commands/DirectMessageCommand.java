    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            if (args.length > 2) {
                ...
                if (target != null) {
                    args[0] = "";
                    target.sendMessage(Utils.chat("&7[&6" + sender.getName() + "&r&7 -> " + target.getName() + "] &7>> " + getMessage(args)));
                    sender.sendMessage(Utils.chat("&7[&6" + sender.getName() + "&r&7 -> &6" + target.getName() + "] &7>> " + getMessage(args)));
                } else return false;
            }
        }
        return true;
